...
arcpy.AddField_management(out_table, "SAP_TPA", "DOUBLE") # Added Sapling TPA
...
LOWER("' + tr_spp + '") <> \'\'') # Modified select query to include all diameters
...
arcpy.CalculateField_management(tree_sap, "DENS", "10 / (0.005454 * !"+tr_dbh+"! * !"+tr_dbh+"!)", "PYTHON")
arcpy.Statistics_analysis(tree_sap, scratch_11a, [["DENS", "SUM"]], case_three)
arcpy.AddField_management(scratch_11a, "JOINFIELD", "TEXT")
arcpy.CalculateField_management(scratch_11a, "JOINFIELD", "!"+str(summary_level)+"! + !" +str(tr_spp)+ "!", "PYTHON")
arcpy.AddIndex_management(scratch_11a, "JOINFIELD", "summary_11a", "NON_UNIQUE", "NON_ASCENDING")
arcpy.JoinField_management(out_table, "JOINFIELD", scratch_11a, "JOINFIELD", ["SUM_DENS"])
arcpy.CalculateField_management(out_table, "SAP_TPA", "1.0 * !SUM_DENS!/!TOT_PL!", "PYTHON")
...
arcpy.TableSelect_analysis(tree_prism, tree_pol, '"'+tr_dbh+'" >= 6 AND "'+tr_dbh+'" <12 AND LOWER("'+tr_hlth+'")<> \'d\'') # modified query
...
arcpy.CalculateField_management(out_table, "AV_TPA", "0.0 + !SAP_TPA! + !POL_TPA! + !SAW_TPA! + !MAT_TPA! + !OVM_TPA! + !SNAG_TPA!", "PYTHON") #added sapling size class
...
arcpy.CalculateField_management(out_table, "SAP_TPA", "removenull(!SAP_TPA!)", "PYTHON", codeblock) # Added sapling size class
arcpy.CalculateField_management(out_table, "POL_TPA", "removenull(!POL_TPA!)", "PYTHON", codeblock)
arcpy.CalculateField_management(out_table, "SAW_TPA", "removenull(!SAW_TPA!)", "PYTHON", codeblock)
arcpy.CalculateField_management(out_table, "MAT_TPA", "removenull(!MAT_TPA!)", "PYTHON", codeblock)
arcpy.CalculateField_management(out_table, "OVM_TPA", "removenull(!OVM_TPA!)", "PYTHON", codeblock)
