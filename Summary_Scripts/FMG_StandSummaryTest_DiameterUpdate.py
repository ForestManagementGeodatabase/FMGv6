...
# 7-20-2021 Update: Remove 4" diameter filter when constructing tree_prism table
#   added appropriate code to include a SAP_TPA field and calculations for trees
#   1"-5" inches in size. Adjusted size diameter class on POL size timber
...
arcpy.AddField_management(out_table, "SAP_TPA", "DOUBLE") # Added sapling tpa field
...
arcpy.AddField_management(out_table, "LG_SNAG_TPA", "DOUBLE") # Added large snag tpa field
...
LOWER("' + tr_spp + '") <> \'\' and LOWER("' +tr_spp + '") <> \'notree\'') # removed diameter from query
...
arcpy.AddMessage('Calculating SAP_TPA field...') # Added this entire section
scratch_11a = env.scratchWorkspace + "\scratch_11a"
tree_sap = str(env.scratchWorkspace)+"\\tree_sap"
arcpy.TableSelect_analysis(tree_prism, tree_sap, '"'+tr_dbh+'" >= 1 AND "'+tr_dbh+'" <6 AND LOWER("'+tr_hlth+'")<> \'d\'')
arcpy.AddField_management(tree_sap, "DENS", "DOUBLE")
arcpy.CalculateField_management(tree_sap, "DENS", "10/(!"+tr_dbh+"!*!"+tr_dbh+"!*0.005454)", "PYTHON")
arcpy.Statistics_analysis(tree_sap, scratch_11a, [["DENS", "SUM"]], summary_level)
arcpy.JoinField_management(out_table, summary_level, scratch_11a, summary_level, ["SUM_DENS"])
arcpy.CalculateField_management(out_table, "SAP_TPA", "1.0 * !SUM_DENS! / !NUM_PL!", "PYTHON")
arcpy.CalculateField_management(out_table, "SAP_TPA", "removenull(!SAP_TPA!)", "PYTHON", codeblock)
arcpy.DeleteField_management(out_table, ["SUM_DENS"])
arcpy.Delete_management(scratch_11a)
arcpy.Delete_management(tree_sap)
...
arcpy.TableSelect_analysis(tree_prism, tree_pol, '"'+tr_dbh+'" >= 6 AND "'+tr_dbh+'" <12 AND LOWER("'+tr_hlth+'")<> \'d\'') # modified this line
...
arcpy.AddMessage('Calculating LG_SNAG_TPA field...') # Added this entire section
scratch_8a = env.scratchWorkspace + "\scratch_8a"
tree_lgsnag = str(env.scratchWorkspace)+"\\tree_lgsnag"
arcpy.TableSelect_analysis(tree_prism, tree_lgsnag, '"'+tr_dbh+'">=20 AND LOWER("'+tr_hlth+'")IN (\'snag\', \'dead\',\'d\')')
arcpy.AddField_management(tree_lgsnag, "DENS", "DOUBLE")
arcpy.CalculateField_management(tree_lgsnag, "DENS", "10/(!"+tr_dbh+"!*!"+tr_dbh+"!*0.005454)", "PYTHON")
arcpy.Statistics_analysis(tree_lgsnag, scratch_8a, [["DENS", "SUM"]], summary_level)
arcpy.JoinField_management(out_table, summary_level, scratch_8a, summary_level, ["SUM_DENS"])
arcpy.CalculateField_management(out_table, "LG_SNAG_TPA", "1.0 * !SUM_DENS! / !NUM_PL!", "PYTHON")
arcpy.CalculateField_management(out_table, "LG_SNAG_TPA", "removenull(!LG_SNAG_TPA!)", "PYTHON", codeblock)
arcpy.DeleteField_management(out_table, ["SUM_DENS"])
arcpy.Delete_management(scratch_8a)
arcpy.Delete_management(tree_lgsnag)

arcpy.AddMessage('Calculating AV_TPA field...')
arcpy.CalculateField_management(out_table, "AV_TPA", "0.0 + 1.0*!SAP_TPA! + 1.0*!POL_TPA! + 1.0*!SAW_TPA! + 1.0*!MAT_TPA! + 1.0*!OVM_TPA! + 1.0*!SNAG_TPA!", "PYTHON") #added SAP field to calculation
...
arcpy.TableSelect_analysis(tree_prism, scratch_9, 'LOWER("'+mast_type+'") = \'hard\' AND LOWER("'+tr_hlth+'")<>\'d\'') #adjusted query syntax
...
arcpy.TableSelect_analysis(tree_prism, scratch_10, 'LOWER("'+mast_type+'") = \'soft\' AND LOWER("'+tr_hlth+'")<>\'d\'') #adjusted query syntax
